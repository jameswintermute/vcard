...
def _status_bar() -> None:
    ...
    if n_src == 0:
        src_body = Text("No files in cards-in/", style=f"dim {_DIM}")
    else:
        src_body = Text()
    ...
    # Checkpoint panel — shown only when a checkpoint exists
    from .checkpoint import checkpoint_info
    ckpt = checkpoint_info(Path("cards-wip"))
    if ckpt:
        reviewed = ckpt.get("review_index", 0)
        total    = ckpt.get("total_cards", "?")
        saved_at = ckpt.get("saved_at", "")[:16].replace("T", "  ")
        ckpt_body = Text()
        ckpt_body.append(f"{reviewed} / {total}", style=f"bold {_AMBER}")
        ckpt_body.append(" cards reviewed   ", style=f"{_TEXT}")
        ckpt_body.append(f"saved {saved_at}", style=f"dim {_DIM}")
        ckpt_body.append("   press ", style=f"dim {_DIM}")
        ckpt_body.append("r", style=f"bold {_AMBER}")
        ckpt_body.append(" to resume", style=f"dim {_DIM}")
        console.print(Panel(ckpt_body, title=Text("CHECKPOINT", style=f"bold {_AMBER}"),
                            title_align="left", border_style=_AMBER, padding=(0, 1)))
    console.print()
...
def _menu() -> None:
    ...
    console.print()
    section("MERGE")
    item("1", "Merge sources → clean output",  "reads cards-in/")
    item("r", "Resume category review",         "continue from checkpoint")
    item("2", "Merge and show diff",            "logs every change")
    item("3", "Dry run",                         "no files written")
    console.print()
    section("OUTPUT")
    item("4", "Open cards-out/ in file manager")
    ...
    console.print()
    section("WEB UI")
    item("w", "Open browser UI", "localhost:8421")
    console.print()
    quit_line = Text()
...
def _open_clean_dir() -> None:
    import os, platform
    clean_dir = Path("cards-out").resolve()
    if not clean_dir.is_dir():
        console.print(Panel(Text("Run a merge first to create cards-out/", style=_AMBER),
...
def _show_output_stats() -> None:
    ...
    clean_dir = Path("cards-out")
    if not clean_dir.is_dir():
        console.print(Text("  cards-out/ not found", style=f"dim {_DIM}"))
        return
    vcfs = sorted(clean_dir.glob("*.vcf"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not vcfs:
...
def _help() -> None:
    ...
    console.print(Panel(
        Text.from_markup(
            ...
            f"  [dim {_DIM}]1.[/]  Drop exported .vcf files into  [bold {_ACCENT}]cards-in/[/]\n"
    ...
    try:
        while True:
            ...
            if choice in {"q", "quit", "exit"}:
                ...
                from .checkpoint import load_checkpoint, checkpoint_info
                ckpt_meta = checkpoint_info(Path("cards-wip"))
                if not ckpt_meta:
                    console.print(Text("  No checkpoint found.", style=f"dim {_DIM}"))
                else:
                    result = load_checkpoint(Path("cards-wip"))
                    if result is None:
                        console.print(Text("  Checkpoint could not be loaded.", style=f"dim {_RED}"))
                    else:
                        cards, meta = result
                        resume_idx = meta.get("review_index", 0)
                        console.print()
                        console.print(Text(f"  Resuming from card {resume_idx + 1} of {meta.get('total_cards', '?')}…", style=f"bold {_TEXT}"))
                        console.print()
                        from .formatters import prompt_categories_interactive, _DELETE_SENTINEL
                        from .exporter import export_vcards
                        from datetime import date
                        import os, sys
                        # Slice to resume position
                        prompt_categories_interactive(cards, work_dir=Path("cards-wip"))
                        # Filter deletions
                        cards = [c for c in cards if _DELETE_SENTINEL not in c.categories]
                        # Save to cards-clean
                        _, settings = __import__("vcard_normalizer.config", fromlist=["ensure_workspace"]).ensure_workspace()
                        iso = date.today().isoformat()
                        out = Path("cards-out") / f"{iso}-Contacts-of-{settings.owner_name.replace(' ', '-')}.vcf"
                        n = export_vcards(cards, out, target_version="4.0")
                        from .checkpoint import clear_checkpoint
                        clear_checkpoint(Path("cards-wip"))
                        console.print(Text(f"  ✓  Wrote {n} contact(s) → {out}", style=f"bold {_GREEN}"))
            elif choice == "1":
                console.print()
                _run("merge", "--yes")
                # Check if user quit early and a checkpoint was saved
                from .checkpoint import checkpoint_info
                ckpt = checkpoint_info(Path("cards-wip"))
                if ckpt:
                    reviewed = ckpt.get("review_index", 0)
                    total    = ckpt.get("total_cards", "?")
                    console.print()
                    console.print(Text(f"  Checkpoint saved: {reviewed}/{total} cards reviewed.", style=f"bold {_AMBER}"))
                    console.print(Text("  Press r from the menu to resume.", style=f"dim {_DIM}"))
            elif choice == "2":
                console.print()
                _run("merge", "--yes", "--diff")
                from .checkpoint import checkpoint_info
                ckpt = checkpoint_info(Path("cards-wip"))
                if ckpt:
                    console.print(Text(f"  Checkpoint saved. Press r to resume.", style=f"bold {_AMBER}"))
            elif choice == "3":
                ...
                console.print(Text("\n  Starting browser UI on http://localhost:8421…\n", style=f"bold {_GREEN}"))
