...
from .interactive import QuitReview
...
def _run_pipeline(
    ...
    try:
        ...
        _quit_early = True
        # Auto-merge remaining unvisited clusters so the checkpoint is complete
        for cluster in clusters:
            already = {id(c) for c in merged}
            for card in cluster:
                if id(card) not in already and not any(id(card) in {id(x) for x in m_cluster}
                        for m_cluster in [cluster]):
                    pass
        # Simpler: just auto-merge any cluster whose cards aren't yet in merged
        merged_ids = {id(c) for c in merged}
        for cluster in clusters:
            if not any(id(card) in merged_ids for card in cluster):
                merged.append(merge_cluster_auto(cluster))
                merged_ids = {id(c) for c in merged}
    ...
    # ── 6.5. Save checkpoint / early exit ────────────────────────────────────
    if _quit_early:
        _work_dir = work_dir or Path("cards-wip")
        save_checkpoint(merged, work_dir=_work_dir, review_index=0,
                        source_files=list(source_counts.keys()),
                        input_count=input_count, duplicate_clusters=len(dup_clusters))
        console.print()
        console.print(f"  [bold #f0a500]Progress saved — {len(merged)} cards in checkpoint.[/]")
        console.print(f"  [dim]Press [bold]r[/bold] from the main menu to resume category review.[/dim]")
        return

    # ── 6.5. Save checkpoint ──────────────────────────────────────────────────
    # Saved immediately after dedup so the user can quit category review and
    # resume later without re-processing from scratch.
    _work_dir = work_dir or Path("cards-wip")
    save_checkpoint(
        merged,
        work_dir=_work_dir,
    ...
    console.print(f"  [dim]Checkpoint saved → {_work_dir}/checkpoint.vcf[/dim]")

    # ── 7. Category review ────────────────────────────────────────────────────
    # Interactive mode: full review of every card with the styled grid UI.
    # Non-interactive mode: lighter pass that only surfaces uncategorised cards.
    if interactive:
        ensure_country_in_addresses(merged)
        prompt_categories_interactive(merged, work_dir=_work_dir)
    elif not dry_run:
        # Non-interactive: only prompt for cards that have no category at all
...
def merge(
    merge_dir: Path = typer.Option(
        Path("cards-in"),
        "--dir", "-d",
        help="Folder containing source .vcf files (default: cards-in/)",
    ),
