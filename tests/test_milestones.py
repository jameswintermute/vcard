...
def test_icloud_double_dot_adr(tmp_path: Path):
    """item1..ADR (double dot) from iCloud exports must not crash the parser."""
    vcf = tmp_path / "icloud.vcf"
    vcf.write_text(
        "BEGIN:VCARD\n"
        "VERSION:3.0\n"
        "FN:Anna Kowalski\n"
        "item1..ADR;TYPE=intl:;;Dluga 105 ;Topola Wielka;;63-421;Poland\n"
        "END:VCARD\n"
    )
    pairs = read_vcards_from_files([vcf])
    assert len(pairs) == 1
    cards = normalize_cards(pairs)
    assert cards[0].fn == "Anna Kowalski"


def test_icloud_single_dot_adr(tmp_path: Path):
    """item1.ADR (single dot, standard property) is fixed and the address kept."""
    vcf = tmp_path / "icloud.vcf"
    vcf.write_text(
        "BEGIN:VCARD\n"
        "VERSION:3.0\n"
        "FN:Bob\n"
        "item1.ADR;TYPE=HOME:;;1 High St;London;;W1A 1AA;United Kingdom\n"
        "END:VCARD\n"
    )
    pairs = read_vcards_from_files([vcf])
    cards = normalize_cards(pairs)
    assert len(cards[0].addresses) == 1
...
def test_icloud_x_extension_dropped(tmp_path: Path):
    ...
    pairs = read_vcards_from_files([vcf])
    assert len(pairs) == 1   # card still parsed


def test_mixed_good_and_bad_lines(tmp_path: Path):
    """A file with both clean and malformed lines parses all clean contacts."""
    ...
    vcf.write_text(
        ...
        "FN:Good Contact\n"
        "TEL:07700900000\n"
        "item1..ADR;TYPE=intl:;;Bad Street;Badtown;;000;Nowhere\n"
        "item2.X-ABADR:xx\n"
        "END:VCARD\n"
        "BEGIN:VCARD\n"
        "VERSION:3.0\n"
